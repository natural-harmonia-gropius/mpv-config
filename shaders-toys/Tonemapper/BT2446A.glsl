...
vec3 bt2446a(vec3 rgb) {
    ...
    const float Lhdr = 1000;
    const float Lsdr = 100;

    const float pHDR = 1 + 32 * pow(Lhdr / 10000, 1 / 2.4);
    const float pSDR = 1 + 32 * pow(Lsdr / 10000, 1 / 2.4);

    const float Yp = log(1 + (pHDR - 1) * Y) / log(pHDR);
    ...
    const float Ysdr = (pow(pSDR, Yc) - 1) / (pSDR - 1);
