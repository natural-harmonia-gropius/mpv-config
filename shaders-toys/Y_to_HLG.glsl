//!HOOK MAIN
//!BIND HOOKED
//!DESC broken: linear cd/m^2 to HLG
...
vec3 Y_2_HLG(vec3 displayLinear) {
    ...
    const vec3 sceneLinear = vec3(
        ...
        Y_d == 0 ? 0 : pow((Y_d - beta) / alpha, (1 - gamma) / gamma) * ((displayLinear.r - beta) / alpha),
        Y_d == 0 ? 0 : pow((Y_d - beta) / alpha, (1 - gamma) / gamma) * ((displayLinear.g - beta) / alpha),
        Y_d == 0 ? 0 : pow((Y_d - beta) / alpha, (1 - gamma) / gamma) * ((displayLinear.b - beta) / alpha)
    );

    // HLG OETF (scene linear to non-linear signal value)
    const vec3 HLG = vec3(
        sceneLinear.r <= 1 / 12 ? sqrt(3 * sceneLinear.r) : a * log(12 * sceneLinear.r - b) + c,
        sceneLinear.g <= 1 / 12 ? sqrt(3 * sceneLinear.g) : a * log(12 * sceneLinear.g - b) + c,
        sceneLinear.b <= 1 / 12 ? sqrt(3 * sceneLinear.b) : a * log(12 * sceneLinear.b - b) + c
    );
