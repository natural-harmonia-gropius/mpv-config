...
vec3 HLG_2_Y(vec3 HLG) {
    ...
    const vec3 sceneLinear = vec3(
        HLG.r >= 0 && HLG.r <= 0.5 ? pow(HLG.r, 2) / 3 : (exp((HLG.r - c) / a) + b) / 12,
        HLG.g >= 0 && HLG.g <= 0.5 ? pow(HLG.g, 2) / 3 : (exp((HLG.g - c) / a) + b) / 12,
        HLG.b >= 0 && HLG.b <= 0.5 ? pow(HLG.b, 2) / 3 : (exp((HLG.b - c) / a) + b) / 12
    );
